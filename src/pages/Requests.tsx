...
const Requests = () => {
  ...
  const { data: requests = [], isLoading } = useQuery({
    queryKey: ['requests', selectedBoatId, statusFilter],
    queryFn: async () => {
      let query = supabase.from('requests').select('*').order('created_at', { ascending: false });
      if (selectedBoatId !== 'all') query = query.eq('boat_id', selectedBoatId);
      if (statusFilter === 'active') query = query.in('status', ['pending', 'in_progress']);
  ...
  const updateStatusMutation = useMutation({
    mutationFn: async ({ id, newStatus }: { id: string; newStatus: string }) => {
      const { error } = await supabase.from('requests').update({ status: newStatus as any }).eq('id', id);
      if (error) throw error;
    },
      ...
      <div className="space-y-6 animate-fade-in">
        <div className="flex flex-col sm:flex-row gap-3">
          <Select value={selectedBoatId} onValueChange={setSelectedBoatId}>
            <SelectTrigger className="w-[180px] bg-secondary/50"><SelectValue placeholder="All boats" /></SelectTrigger>
            <SelectContent>
          ...
          <Select value={statusFilter} onValueChange={setStatusFilter}>
            <SelectTrigger className="w-[180px] bg-secondary/50"><SelectValue /></SelectTrigger>
            <SelectContent>
                    ...
                    <div className="flex items-start justify-between gap-4">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap mb-2">
                          ...
                          <Badge variant="outline" className="text-xs">{getBoatName(req.boat_id)}</Badge>
                          <span className="text-xs text-muted-foreground">{formatTime(req.created_at)}</span>
                        </div>
                        <p className="text-sm font-medium mb-1">{categoryLabels[req.category] || req.category}</p>
                        {req.translated_message && <p className="text-sm text-foreground">{req.translated_message}</p>}
                        {req.original_message && req.guest_language !== 'en' && (
                          <p className="text-xs text-muted-foreground mt-1 italic">Original ({req.guest_language}): {req.original_message}</p>
                        )}
                      </div>
                      <div className="flex flex-col items-end gap-2">
                        ...
                        {req.status === 'pending' && (
                          <Button size="sm" variant="outline" onClick={() => updateStatusMutation.mutate({ id: req.id, newStatus: 'in_progress' })}>Start</Button>
                        )}
                        {req.status === 'in_progress' && (
                          <Button size="sm" onClick={() => updateStatusMutation.mutate({ id: req.id, newStatus: 'done' })}>Done</Button>
                        )}
                      </div>
                    </div>
                  </CardContent>
